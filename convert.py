...
class Converter(nn.Module):
    def __init__(self, opt: Options, ply_path):
        super().__init__()
        ...
        self.gaussians = self.gs_renderer.load_ply(ply_path).to(self.device)
    ...
    def render_gs(self, pose):
        ...
        cam_view = torch.inverse(cam_poses).transpose(1, 2).contiguous() # [V, 4, 4]
...
def convert_ply_to_glb(in_path, out_path):
    if os.path.exists(out_path.replace('.ply', '.glb')):
        print(f'{out_path} already exists, skip.')
        return
    
    print(f'convert {in_path}....')
    # load a saved ply and convert to mesh
    assert in_path.endswith('.ply'), '--test_path must be a .ply file saved by infer.py'

    converter = Converter(opt,in_path).cuda()
    converter.fit_nerf()
    converter.fit_mesh()
    converter.fit_mesh_uv()
    converter.export_mesh(out_path.replace('.ply', '.glb'))
    print(f'saved to {out_path}')



if os.path.isdir(opt.test_path):
    if not os.path.exists(opt.test_path+'/glb'):
        os.makedirs(opt.test_path+'/glb')
    for ply_file in os.listdir(opt.test_path):
        if ply_file.endswith('.ply'):
            convert_ply_to_glb(os.path.join(opt.test_path, ply_file), os.path.join(opt.test_path, 'glb', ply_file))
else:
    dir='/'.join(opt.test_path.split('/')[:-1])
    file=opt.test_path.split('/')[-1]
    if not os.path.exists(dir+'/glb'):
        os.makedirs(dir+'/glb')
    convert_ply_to_glb(opt.test_path, os.path.join(dir, 'glb', file))
