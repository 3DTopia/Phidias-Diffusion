...
class CrossAttention(nn.Module):
    ...
    def forward(self, q: Tensor, k: Tensor, v: Tensor) -> Tensor:
        ...
        q = self.scale * self.to_q(q).reshape(B, N, self.num_heads, self.dim // self.num_heads).permute(0, 2, 1, 3).contiguous() # [B, nh, N, C/nh]
        k = self.to_k(k).reshape(B, M, self.num_heads, self.dim // self.num_heads).permute(0, 2, 1, 3).contiguous() # [B, nh, M, C/nh]
        v = self.to_v(v).reshape(B, M, self.num_heads, self.dim // self.num_heads).permute(0, 2, 1, 3).contiguous() # [B, nh, M, C/nh]

        attn = q @ k.transpose(-2, -1).contiguous() # [B, nh, N, M]

        attn = attn.softmax(dim=-1) # [B, nh, N, M]
        attn = self.attn_drop(attn)

        x = (attn @ v).transpose(1, 2).contiguous().reshape(B, N, -1) # [B, nh, N, M] @ [B, nh, M, C/nh] --> [B, nh, N, C/nh] --> [B, N, nh, C/nh] --> [B, N, C]
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
